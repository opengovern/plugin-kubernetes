	...
	return &plugin.Table{
		...
		Columns: commonColumns([]*plugin.Column{
			//// PodSpec Columns
			{
				...
				Transform:   transform.FromField("Description.Pod.Spec.Volumes"),
			},
			{
				Name:        "containers",
				Type:        proto.ColumnType_JSON,
			...
			//},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.EphemeralContainers"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.InitContainers"),
			},
			{
				Name:        "restart_policy",
				Type:        proto.ColumnType_STRING,
				Description: "Restart policy for all containers within the pod. One of Always, OnFailure, Never.",
				Transform:   transform.FromField("Description.Pod.Spec.RestartPolicy"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.TerminationGracePeriodSeconds"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.ActiveDeadlineSeconds"),
			},
			{
				Name:        "dns_policy",
				Type:        proto.ColumnType_STRING,
				Description: "DNS policy for pod.  Valid values are 'ClusterFirstWithHostNet', 'ClusterFirst', 'Default' or 'None'.",
				Transform:   transform.FromField("Description.Pod.Spec.DNSPolicy"),
			},
			{
				Name:        "node_selector",
				Type:        proto.ColumnType_JSON,
				Description: "NodeSelector is a selector which must be true for the pod to fit on a node.",
				Transform:   transform.FromField("Description.Pod.Spec.NodeSelector"),
			},
			{
				Name:        "service_account_name",
				Type:        proto.ColumnType_STRING,
				Description: "ServiceAccountName is the name of the ServiceAccount to use to run this pod.",
				Transform:   transform.FromField("Description.Pod.Spec.ServiceAccountName"),
			},
			{
				Name:        "automount_service_account_token",
				Type:        proto.ColumnType_BOOL,
				Description: "AutomountServiceAccountToken indicates whether a service account token should be automatically mounted.",
				Transform:   transform.FromField("Description.Pod.Spec.AutomountServiceAccountToken"),
			},
			{
				Name: "node_name",
				Type: proto.ColumnType_STRING,
				Description: "NodeName is a request to schedule this pod onto a specific node. If it is non-empty, " +
					...
					"requirements.",
				Transform: transform.FromField("Description.Pod.Spec.NodeName"),
			},
			{
				Name: "host_network",
				Type: proto.ColumnType_BOOL,
				...
				Transform: transform.FromField("Description.Pod.Spec.HostNetwork"),
			},
			{
				Name:        "host_pid",
				Type:        proto.ColumnType_BOOL,
				Description: "Use the host's pid namespace.",
				Transform:   transform.FromField("Description.Pod.Spec.HostPID"),
			},
			{
				Name:        "host_ipc",
				Type:        proto.ColumnType_BOOL,
				Description: "Use the host's ipc namespace.",
				Transform:   transform.FromField("Description.Pod.Spec.HostIPC"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.ShareProcessNamespace"),
			},
			{
				Name:        "security_context",
				Type:        proto.ColumnType_JSON,
				Description: "SecurityContext holds pod-level security attributes and common container settings.",
				Transform:   transform.FromField("Description.Pod.Spec.SecurityContext"),
			},

			{
				...
				Transform:   transform.FromField("Description.Pod.Spec.ImagePullSecrets"),
			},
			{
				Name:        "hostname",
				Type:        proto.ColumnType_STRING,
				Description: "Specifies the hostname of the Pod. If not specified, the pod's hostname will be set to a system-defined value.",
				Transform:   transform.FromField("Description.Pod.Spec.Hostname"),
			},
			{
				Name: "subdomain",
				Type: proto.ColumnType_STRING,
				...
				Transform: transform.FromField("Description.Pod.Spec.Subdomain"),
			},
			{
				Name:        "affinity",
				Type:        proto.ColumnType_JSON,
				Description: "If specified, the pod's scheduling constraints.",
				Transform:   transform.FromField("Description.Pod.Spec.Affinity"),
			},
			{
				Name:        "scheduler_name",
				Type:        proto.ColumnType_STRING,
				Description: "If specified, the pod will be dispatched by specified scheduler.",
				Transform:   transform.FromField("Description.Pod.Spec.SchedulerName"),
			},
			{
				Name:        "tolerations",
				Type:        proto.ColumnType_JSON,
				Description: "If specified, the pod's tolerations.",
				Transform:   transform.FromField("Description.Pod.Spec.Tolerations"),
			},
			{
				Name: "host_aliases",
				Type: proto.ColumnType_JSON,
				...
				Transform: transform.FromField("Description.Pod.Spec.HostAliases"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.PriorityClassName"),
			},
			{
				Name: "priority",
				Type: proto.ColumnType_INT,
				...
				Transform: transform.FromField("Description.Pod.Spec.Priority"),
			},
			{
				Name: "dns_config",
				Type: proto.ColumnType_JSON,
				...
				Transform: transform.FromField("Description.Pod.Spec.DNSConfig"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.ReadinessGates"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.RuntimeClassName"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.EnableServiceLinks"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.PreemptionPolicy"),
			},
			{
				Name:        "overhead",
				Type:        proto.ColumnType_JSON,
				Description: "Overhead represents the resource overhead associated with running a pod for a given RuntimeClass.",
				Transform:   transform.FromField("Description.Pod.Spec.Overhead"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.TopologySpreadConstraints"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Spec.SetHostnameAsFQDN"),
			},
			...
			//// PodStatus Columns
			{
				...
				Transform: transform.FromField("Description.Pod.Status.Phase"),
			},
			{
				Name:        "conditions",
				Type:        proto.ColumnType_JSON,
				Description: "Current service state of pod.",
				Transform:   transform.FromField("Description.Pod.Status.Conditions"),
			},

			{
				...
				Transform:   transform.FromField("Description.Pod.Status.Message"),
			},
			{
				Name:        "status_reason",
				Type:        proto.ColumnType_STRING,
				Description: "A brief CamelCase message indicating details about why the pod is in this state. e.g. 'Evicted'",
				Transform:   transform.FromField("Description.Pod.Status.Reason"),
			},
			{
				...
				Description: "nominatedNodeName is set only when this pod preempts other pods on the node, but it cannot be " +
					...
					"scheduled.",
				Transform: transform.FromField("Description.Pod.Status.NominatedNodeName"),
			},
			{
				Name:        "host_ip",
				Type:        proto.ColumnType_IPADDR,
				Description: "IP address of the host to which the pod is assigned. Empty if not yet scheduled.",
				Transform:   transform.FromField("Description.Pod.Status.HostIP"),
			},

			{
				...
				Transform: transform.FromField("Description.Pod.Status.PodIP"),
			},
			{
				Name: "pod_ips",
				Type: proto.ColumnType_JSON,
				...
				Transform: transform.FromField("Description.Pod.Status.PodIPs"),
			},

			{
				...
				Transform: transform.FromField("Description.Pod.Status.StartTime").Transform(v1TimeToRFC3339),
			},

			{
				...
				Description: "The list has one entry per init container in the manifest. The most recent successful " +
					...
					"startTime set.",
				Transform: transform.FromField("Description.Pod.Status.InitContainerStatuses"),
			},
			{
				...
				Description: "The list has one entry per container in the manifest. Each entry is currently the output " +
					"of `docker inspect`.",
				Transform: transform.FromField("Description.Pod.Status.ContainerStatuses"),
			},
			{
				Name:        "qos_class",
				Type:        proto.ColumnType_STRING,
				Description: "The Quality of Service (QOS) classification assigned to the pod based on resource requirements.",
				Transform:   transform.FromField("Description.Pod.Status.QOSClass"),
			},
			{
				...
				Transform: transform.FromField("Description.Pod.Status.EphemeralContainerStatuses"),
			},
			...
			//// Steampipe Standard Columns
			{
				...
				Transform:   transform.FromField("Description.Pod.Name"),
			},
			{
				Name:        "tags",
				Type:        proto.ColumnType_JSON,
...
func transformPodTags(_ context.Context, d *transform.TransformData) (interface{}, error) {
	obj := d.HydrateItem.(opengovernance.KubernetesPod).Description.Pod
